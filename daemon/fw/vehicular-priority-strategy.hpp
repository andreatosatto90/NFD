 ...
 */
class VehicularPriorityStrategy : public Strategy
{
public:
  VehicularPriorityStrategy(Forwarder& forwarder, const Name& name = STRATEGY_NAME);

  virtual
  ~VehicularPriorityStrategy();
...
private: // forwarding procedures
  ...
  // TODO mio Mie aggiunte

  void
  handleFaceDown(const Face& inFace,
                 const Interest& interest,
                 shared_ptr<fib::Entry> fibEntry,
                 shared_ptr<pit::Entry> pitEntry,
                 const shared_ptr<nfd::face::Face>& failedFace);
...
private:
  ...
  std::map<shared_ptr<pit::Entry>, signal::Connection> m_sentInterests;
};
