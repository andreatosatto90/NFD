...
#include "vehicular-priority-strategy.hpp"
#include "core/logger.hpp"

namespace nfd {
namespace fw {

NFD_LOG_INIT("VehicularPriorityStrategy");

const Name VehicularPriorityStrategy::STRATEGY_NAME("ndn:/localhost/nfd/strategy/vehicular-priority/%FD%01");
NFD_REGISTER_STRATEGY(VehicularPriorityStrategy);

VehicularPriorityStrategy::VehicularPriorityStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder, name)
  , m_removeFaceInfoConn(this->beforeRemoveFace.connect(
                         bind(&VehicularPriorityStrategy::removeFaceInfo, this, _1)))
{
}

VehicularPriorityStrategy::~VehicularPriorityStrategy()
{
}

void
VehicularPriorityStrategy::afterReceiveInterest(const Face& inFace,
...
void
VehicularPriorityStrategy::afterReceiveNewInterest(const Face& inFace,
                                                   const Interest& interest,
                                                   shared_ptr<fib::Entry> fibEntry,
                                                   shared_ptr<pit::Entry> pitEntry)
{
  ...
  // TODO roba mia, invia a tutti
  NFD_LOG_DEBUG("Eligible next faces number: " <<nexthops.size());
  for (fib::NextHopList::const_iterator it = nexthops.begin(); it != nexthops.end(); ++it) {
    shared_ptr<Face> outFace = it->getFace();
    if (pitEntry->canForwardTo(*outFace) &&
        outFace->getState() == nfd::face::TransportState::UP) {
      NFD_LOG_DEBUG("Sending to " << outFace->getId());


      m_sentInterests.insert(std::make_pair(pitEntry, outFace->afterStateChange.connect(
                                                        bind(&VehicularPriorityStrategy::handleFaceDown, this,
                                                        boost::cref(inFace), boost::cref(interest),
                                                        fibEntry, pitEntry, outFace))));
...
void
VehicularPriorityStrategy::afterReceiveRetxInterest(const Face& inFace,
                                         const Interest& interest,
                                         shared_ptr<fib::Entry> fibEntry,
                                         shared_ptr<pit::Entry> pitEntry)
{
...
bool
VehicularPriorityStrategy::sendToLastNexthop(const Face& inFace, shared_ptr<pit::Entry> pitEntry, MtInfo& mi,
                                  shared_ptr<fib::Entry> fibEntry)
{
  ...
  pi->rtoTimer = scheduler::schedule(rto,
      bind(&VehicularPriorityStrategy::afterRtoTimeout, this, weak_ptr<pit::Entry>(pitEntry),
...
void
VehicularPriorityStrategy::afterRtoTimeout(weak_ptr<pit::Entry> pitWeak, weak_ptr<fib::Entry> fibWeak,
                                FaceId inFace, FaceId firstOutFace)
{
  ...
  auto it = m_sentInterests.find(pitEntry);
  if (it != m_sentInterests.end()) {
    NFD_LOG_TRACE("Disconnection state changed");
...
void
VehicularPriorityStrategy::multicast(shared_ptr<pit::Entry> pitEntry, shared_ptr<fib::Entry> fibEntry,
...
void
VehicularPriorityStrategy::beforeSatisfyInterest(shared_ptr<pit::Entry> pitEntry,
...
void VehicularPriorityStrategy::beforeExpirePendingInterest(shared_ptr<pit::Entry> pitEntry)
{
  auto it = m_sentInterests.find(pitEntry);
  if (it != m_sentInterests.end()) {
    NFD_LOG_TRACE("Disconnection state changed");
...
void VehicularPriorityStrategy::afterReceiveNack(const nfd::face::Face& inFace,
                                                 const lp::Nack& nack,
                                                 shared_ptr<fib::Entry> fibEntry,
                                                 shared_ptr<pit::Entry> pitEntry)
{
...
void
VehicularPriorityStrategy::updateMeasurements(const Face& inFace, const Data& data,
...
// TODO mio solo per test per ora
void VehicularPriorityStrategy::handleFaceDown(const nfd::face::Face& inFace,
                                               const ndn::Interest& interest,
                                               shared_ptr<fib::Entry> fibEntry,
                                               shared_ptr<pit::Entry> pitEntry,
                                               const shared_ptr<Face>& failedFace)
{
  NFD_LOG_TRACE("Face state changed");
  ...
  // TODO mio disconnessione segnale
  afterReceiveNewInterest(inFace, interest, fibEntry, pitEntry);


}

VehicularPriorityStrategy::MtInfo::MtInfo()
  : lastNexthop(face::INVALID_FACEID)
  , rtt(1, time::milliseconds(1), 0.1)
{
}

std::tuple<Name, shared_ptr<VehicularPriorityStrategy::MtInfo>>
VehicularPriorityStrategy::findPrefixMeasurements(const pit::Entry& pitEntry)
{
...
shared_ptr<VehicularPriorityStrategy::MtInfo>
VehicularPriorityStrategy::addPrefixMeasurements(const Data& data)
{
...
VehicularPriorityStrategy::FaceInfo::FaceInfo()
  : rtt(1, time::milliseconds(1), 0.1)
{
}

void
VehicularPriorityStrategy::removeFaceInfo(shared_ptr<Face> face)
{
